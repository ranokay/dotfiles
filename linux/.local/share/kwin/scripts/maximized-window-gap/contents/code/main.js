	...
	return {
		left: {
			...
			quarterLeft: {
				...
				gapped: Math.round(
					area.left + (1 * (area.width + config.gapLeft - config.gapRight + config.gapMid)) / 4
				),
			},
			halfHorizontal: {
				closed: Math.round(area.left + area.width / 2),
				gapped: Math.round(
					area.left + (area.width + config.gapLeft - config.gapRight + config.gapMid) / 2
				),
			},
			quarterRight: {
				closed: Math.round(area.left + 3 * (area.width / 4)),
				gapped: Math.round(
					area.left + (3 * (area.width + config.gapLeft - config.gapRight + config.gapMid)) / 4
				),
			},
		},
		right: {
			quarterLeft: {
				...
				gapped: Math.round(
					area.right - (3 * (area.width + config.gapLeft - config.gapRight + config.gapMid)) / 4
				),
			},
			halfHorizontal: {
				closed: Math.round(area.right - area.width / 2),
				gapped: Math.round(
					area.right - (area.width + config.gapLeft - config.gapRight + config.gapMid) / 2
				),
			},
			quarterRight: {
				closed: Math.round(area.right - area.width / 4),
				gapped: Math.round(
					area.right - (1 * (area.width + config.gapLeft - config.gapRight + config.gapMid)) / 4
				),
			},
			fullRight: {
		...
		top: {
			...
			quarterTop: {
				...
				gapped: Math.round(
					area.top + (1 * (area.height + config.gapTop - config.gapBottom + config.gapMid)) / 4
				),
			},
			halfVertical: {
				closed: Math.round(area.top + area.height / 2),
				gapped: Math.round(
					area.top + (area.height + config.gapTop - config.gapBottom + config.gapMid) / 2
				),
			},
			quarterBottom: {
				closed: Math.round(area.top + 3 * (area.height / 4)),
				gapped: Math.round(
					area.top + (3 * (area.height + config.gapTop - config.gapBottom + config.gapMid)) / 4
				),
			},
		},
		bottom: {
			quarterTop: {
				...
				gapped: Math.round(
					area.bottom - (3 * (area.height + config.gapTop - config.gapBottom + config.gapMid)) / 4
				),
			},
			halfVertical: {
				closed: Math.round(area.bottom - area.height / 2),
				gapped: Math.round(
					area.bottom - (area.height + config.gapTop - config.gapBottom + config.gapMid) / 2
				),
			},
			quarterBottom: {
				closed: Math.round(area.bottom - area.height / 4),
				gapped: Math.round(
					area.bottom - (1 * (area.height + config.gapTop - config.gapBottom + config.gapMid)) / 4
				),
			},
			fullBottom: {
...
function nearArea(actual, expected, gap) {
	return (
		(Math.abs(actual - expected.closed) <= 2 * gap ||
...
function ignoreClient(client) {
	return (
		...
		(config.excludeMode && config.excludedApps.includes(String(client.resourceClass))) ||
		(config.includeMode && !config.includedApps.includes(String(client.resourceClass)))
	)
}
