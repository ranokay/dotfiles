...
def get_mapping(conn, table):
    ...
    :param conn: The SQLite connection.
    :param table: The table to obtain a mapping for (should be apps, or downloading_apps)
...
def setup_items(conn, type_, layout, mapping, group_id, root_parent_id):
    ...
    :param conn: The SQLite connection.
    :param type_: The type of item being manipulated (usually Types.APP)
...
def build_launchpad(config, rebuild_db=True, restart_upon_completion=True):
    """
    Builds the requested layout for the Launchpad apps by updating
...
def build_layout(root, parent_mapping):
    ...
    for page_id, _, _, _ in parent_mapping[root]:
        ...
        # Iterate through items
        for id, type_, app_title, group_title in parent_mapping[page_id]:
...
def extract_launchpad():
    ...
    while True:
        ...
        id, parent_id, type_, app_title, group_title = row
        parent_mapping[parent_id].append((id, type_, app_title, group_title))
